...
boolean mainState, mainStateLast, mainStateSend, acqState, acqStateSend, doorState, doorStateLast, doorStateSend, loadState, tempState, tempStateSend;
...
#define SYS_SMS_AC_OUT                      30
#define SYS_SMS_AC_OUT_SENDING              31
#define SYS_SMS_AC_IN                       32
#define SYS_SMS_AC_IN_SENDING               33
#define SYS_SMS_INFOR_OUT                   34
#define SYS_SMS_INFOR_OUT_SENDING           35
#define SYS_SMS_DOOR_OUT                    36
#define SYS_SMS_DOOR_OUT_SENDING            37
...
int checkBuff(){    
    ...
    if(startCheckingTime != 0){
        switch(systemState){
            case SYS_SMS_AC_OUT_SENDING:                                                             //SMS AC OUT
                if ((currentMillis - startCheckingTime) < SMS_INTERVAL) {
                    ...
                    systemState = SYS_SMS_AC_OUT;
                    networkError++;
                    if(networkError >= 2){
                        #ifdef GSM_SMS_ERROR
                        for(int i = 0; i < 200; i++){ 
                           displayLed7(99.99, LED7_GSM_SMS_ERROR);
                        }
                        #endif
                        Gsm_Init();  
                        systemState = SYS_SMS_AC_OUT;
            ...
            case SYS_SMS_AC_IN_SENDING:                                                             //SMS AC IN
                if ((currentMillis - startCheckingTime) < SMS_INTERVAL) {
                    outputInfor = strstr (RxBuff,"OK");
                    if(outputInfor){    
                        systemState = SYS_MQTT_CONNECT_AT; 
...
int readInput(char indexSource){
    ...
    switch(indexSource){
        case MAIN_SOURCE:
            V_In = digitalRead(INPUT_MAIN_PULSE);
            if (V_In == AC_DISABLE){
                mainState = false;          
                return 1;
            }
...
void getSensorValue(void){  
  ...
  readInput(MAIN_SOURCE);
  int doorValue = readInput(INPUT_DOOR);

  if(doorValue == DOOR_OPEN){
      doorState = false;  
  }
  else{
      doorState = true;      
  ...
  if(doorValue == DOOR_OPEN){
      ...
      doorState = true; 
      if(doorStateSend == false){
          doorStateSend = true; 
      }
  }

  if((doorState == false)&&(doorStateSend == true)){
        //systemState = SYS_SMS_INFOR_OUT_DOOR;
        systemState = SYS_SMS_INFOR_OUT;
  }
  #endif
  */
...
void fanControl(){
  ...
  if(tempState == true){
      if((tempValue > temp_H_Config)||(tempValue < temp_L_Config)){
          ...
          #ifdef WARN_TEMP 
          if(tempStateSend == true){              
              //systemState = SYS_SMS_INFOR_OUT_TEMP;       
              systemState = SYS_SMS_INFOR_OUT;                     
              tempStateSend = false;
          }
          #endif
      ...
      if(tempValue <= TEMP_LOW_TH){
          tempState = true;
          digitalWrite(OUTPUT_LOAD, LOAD_DEACTIVE); 
          #ifdef WARN_TEMP 
          if(tempStateSend == false){              
              //systemState = SYS_SMS_INFOR_OUT_TEMP;                           
              systemState = SYS_SMS_INFOR_OUT;                           
              tempStateSend = true;
          }
...
void relayControl(){
  if(acqState == true){
      if((acqValue > vAcq_H_Config)||(acqValue < vAcq_L_Config)){
          ...
          #ifdef WARN_ACQ
          if(acqStateSend == true){               
              //systemState = SYS_SMS_INFOR_OUT_ACQ;       
              systemState = SYS_SMS_INFOR_OUT;                     
              acqStateSend = false;
          }
          #endif
...
void outSigControl(){
  if((mainState == false)&&(mainStateSend==false)){
  ...
  if((mainState == true)&&(mainStateSend==true)){
...
void sendSmsTaskFunction(){
  switch(systemState){
     case SYS_SMS_AC_OUT:
         ...
         systemState = SYS_SMS_AC_OUT_SENDING;
         mainStateSend = false;
         break;
     ...
     case SYS_SMS_AC_IN:
         ...
         smsInfor();
         GsmMakeSmsChar(msgChar);
         systemState = SYS_SMS_AC_IN_SENDING;
         mainStateSend = false;
         break;
     ...
     case SYS_SMS_INFOR_OUT:
         ...
         if(doorStateSend = true){
              doorStateSend = false;    
         }
         break;

     case SYS_SMS_DOOR_OUT:
         startCheckingTime = 0;
         startMqttCheckingTime = 0;
         strcpy_P(msgChar, (char*)pgm_read_word(&(string_table[2])));
         smsInfor();
